...
class TopCenterParts:Parts{
    ...
    override init(superView:UIView,targetImageView:PictureView,dotView:DotView) {
        ...
        self.frame = CGRectMake(targetImageView.frame.origin.x + targetImageView.frame.width/2 - self.width/2,targetImageView.frame.origin.y - Parts.marginVertical - self.height,self.width,self.height)
    }
    ...
    override func drawRect(rect: CGRect) {
        let line = UIBezierPath(rect:CGRectMake(self.frame.width * 2/5,self.frame.height * 3/5 + self.frame.height/10,self.frame.width/5,self.frame.height/5))
    ...
    func pictureMove() {
        ...
        self.frame.origin.y = self.targetImageView!.frame.origin.y - Parts.marginVertical - self.height
    }
}