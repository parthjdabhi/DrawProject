...
//  アウリクラリア
//
//  Created by 木耳ちゃん on 2016/05/23.
...
class RightTopParts:Parts{
    ...
    override init(superView:UIView,targetImageView:PictureView,dotView:DotView) {
        ...
        self.frame = CGRectMake(targetImageView.frame.origin.x + targetImageView.frame.width + Parts.marginHorizontal,targetImageView.frame.origin.y - Parts.marginVertical - self.height,self.width,self.height)
    ...
    override func drawRect(rect: CGRect) {
        let line = UIBezierPath(rect:CGRectMake(self.frame.width * 1/5 - self.frame.width/10,self.frame.height * 3/5 + self.frame.height/10,self.frame.width/5,self.frame.height/5))
    ...
    func pictureMove() {
        self.frame.origin.x = targetImageView!.frame.origin.x + targetImageView!.frame.width + Parts.marginHorizontal
